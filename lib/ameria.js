...
class AMERIA extends BaseGateway {
  ...
  _normalizeDetails = (data = {}) => {
    ...
    return {
      ...
      paymentAmountInfo: {
        depositedAmount,
        approvedAmount,
        paymentState: data.PaymentState || paymentStateByOrderStatus[orderStatus],
  ...
  _paymentPageResponse = (registerResponse, language) => {
    ...
    return {
      ...
      data: {
        ...
        orderId: paymentId,
        formUrl: this._payUrl(paymentId, language),
        errorCode: 0,
      },
    };
  };
  ...
  attachCard = async (order) => {
    ...
    return this._paymentPageResponse(registerResponse, order.language);
  };

  // Hosted payment for the order amount. No CardHolderID, so Ameria does not save a card.
  initDirectPayment = async (order) => {
    const registerResponse = await this._initPayment({
      amount: order.amount,
  ...
  payOrder = async (order) => {
    ...
    const loadDetails = async () => {
      ...
      return { normalized: this._normalizeDetails(details.data) };
    };

    let loaded = await loadDetails();
    if (loaded.error) return loaded.error;

    // This merchant preauthorizes. Deposit the approved amount so payOrder completes.
    if (loaded.normalized.orderStatus === ORDER_STATUS.APPROVED) {
      const confirmed = await this._confirmPayment({ orderId: paymentId, amount: order.amount });
      if (confirmed.hasError || !this._isSuccessResponseCode(confirmed.data?.ResponseCode)) {
        confirmed.hasError = true;
        confirmed.errorStep = 'ConfirmPayment';
        return { ...res, confirm: confirmed.data, ...confirmed };
      }
      res.confirm = confirmed.data;
      loaded = await loadDetails();
      if (loaded.error) return loaded.error;
    }

    res = Object.assign(res, { hasError: false, data: loaded.normalized });
    if (loaded.normalized.orderStatus !== ORDER_STATUS.DEPOSITED) {
      res.hasError = true;
