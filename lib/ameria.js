...
class AMERIA extends BaseGateway {
  ...
  _makeBindingPayment = async (order) => {
    const payload = {
      ...
      Opaque: order.opaque,
      PaymentType: order.paymentType != null ? order.paymentType : PAYMENT_TYPE_BINDING,
    };
  ...
  _getBindings = async () => {
    return await this._requestToBank('GetBindings', { PaymentType: PAYMENT_TYPE_BINDING });
  };

  _deactivateBinding = async (cardHolderId) => {
